...
exports.DraxList = ({ data, style, itemStyles, renderItemContent, renderItemHoverContent, onItemDragStart, onItemDragPositionChange, onItemDragEnd, onItemReorder, id: idProp, reorderable: reorderableProp, draggable: draggableProp, onScroll: onScrollProp, ...props }) => {
    ...
    react_1.useEffect(() => {
        ...
        if (itemMeasurements.length > itemCount) {
            ...
            while (itemMeasurements.length < itemCount) {
                ...
                shifts.push({
                    targetValue: { x: 0, y: 0 },
                    animatedValue: new react_native_1.Animated.ValueXY({ x: 0, y: 0 }),
                });
            }
        }
    }, [itemCount]);
    ...
    const getShiftTransformStyle = react_1.useCallback((index) => {
    ...
    const renderItem = react_1.useCallback((info) => {
        ...
        return (react_1.default.createElement(DraxView_1.DraxView, Object.assign({ style: [itemStyle, getShiftTransformStyle(originalIndex)], draggingStyle: draggingStyle, dragReleasedStyle: dragReleasedStyle }, otherStyleProps, { payload: { index, originalIndex }, onDragEnd: resetDraggedItem, onDragDrop: resetDraggedItem, draggable: draggable, onMeasure: (measurements) => {
                ...
                && ((hoverContentProps) => renderItemHoverContent(info, hoverContentProps)), longPressDelay: params_1.defaultListItemLongPressDelay })));
    }, [
        originalIndexes,
    ...
    const updateShifts = react_1.useCallback(({ index: fromIndex, originalIndex: fromOriginalIndex }, { index: toIndex }, horizontalShift) => {
        const contentSize = contentSizeRef.current;
    ...
    const onMonitorDragOver = react_1.useCallback((eventData) => {
        const { dragged, receiver, monitorOffsetRatio, dragTranslation, } = eventData;
    ...
    return (react_1.default.createElement(DraxView_1.DraxView, { id: id, style: style, scrollPositionRef: scrollPositionRef, onMeasure: onMeasureContainer, onMonitorDragStart: onMonitorDragStart, onMonitorDragOver: onMonitorDragOver, onMonitorDragExit: onMonitorDragExit, onMonitorDragEnd: onMonitorDragEnd, onMonitorDragDrop: onMonitorDragDrop },
        react_1.default.createElement(DraxSubprovider_1.DraxSubprovider, { parent: { id, nodeHandleRef } },
            react_1.default.createElement(react_native_1.FlatList, Object.assign({}, props, { ref: setFlatListRefs, renderItem: renderItem, onScroll: (scrollEvent) => {
                    onScroll(scrollEvent);
                    if (onScrollProp !== undefined)
                        onScrollProp(scrollEvent);
